...
def test_booking_tracking_flow_cannot_exceed(
    ...
    with client.session_transaction() as sess:
        sess.clear()
        sess['booking_tracker'] = [
            {"name": comp["name"], "alreadyBooked": "0"} for comp in competitions]
    ...
    with client.session_transaction() as sess:
        ...
        spring_booking = next(
            b for b in tracker if b['name'] == 'Winter SkiLïft')
        # Should not have changed
        assert spring_booking['alreadyBooked'] == '5'


def test_multiple_competitions_tracking(
        mocked_client, mocked_clubs_data, mocked_competitions_data):
    ...
    with client.session_transaction() as sess:
        ...
        spring_booking = next(
            (b for b in tracker if b['name'] == 'Spring Festival'), None)
        winter_booking = next(
            (b for b in tracker if b['name'] == 'Winter SkiLïft'), None)
        summer_booking = next(
            (b for b in tracker if b['name'] == 'SummerIs Magic'), None)
