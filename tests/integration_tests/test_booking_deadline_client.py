def test_booking_deadline_client(
    ...
    # Ensure "Book Places" link is not present in the response for
    # competition[4]
    assert response.status_code == 200
    assert f'<a href="/book/{
        competitions[4]["name"]}/Simply Lift">' not in response.data.decode('utf-8')
